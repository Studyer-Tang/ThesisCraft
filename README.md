...
1. 在发布页下载 `ThesisCraft.v4.0.1.Windows-portable.zip`，解压到固定文件夹，无需安装 Python。
...
插件使用前先打开并保存本地文档，再启动对应入口。独立的“学研排版”操作窗口会显示当前连接的文档名，提供快速论文排版、论文设置、插入编号/引用、更新目录与引用、导出 PDF。多个 Office 窗口同时打开时，优先连接最近查看的文档窗口；切换独立实例后，点击“连接当前文档”。即使功能区未显示传统工具栏，也能在操作窗口中使用这些功能。插件进程需要保持运行；可在工作台中自愿启用登录后自动连接。

当前插件是 **Windows 外部 COM 插件（独立操作窗口与兼容工具栏）**，不是 Office 商店加载项。无需开启宏或注册 DLL。仓库中的标准 COM DLL 是实验入口，其自动加载尚未解决。[插件说明](addins/windows/README.md)
