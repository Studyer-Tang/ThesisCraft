...
1. 在发布页下载 `ThesisCraft.v4.0.0.Windows-portable.zip`，解压到固定文件夹，无需安装 Python。
...
| `Word-Plugin.cmd` | 连接 Windows 桌面版 Word 工具栏 |
| `WPS-Plugin.cmd` | 连接 Windows 桌面版 WPS 工具栏 |
| `Desktop.cmd` | 通用桌面程序，支持批量公文排版 |

插件使用前先打开并保存本地文档，再启动对应入口。工具栏提供快速论文排版、论文设置、插入编号/引用、更新目录与引用、导出 PDF。插件进程需要保持运行；可在工作台中自愿启用登录后自动连接。

当前插件是 **Windows 外部 COM 工具栏**，不是 Office 商店加载项。无需开启宏或注册 DLL。仓库中的标准 COM DLL 是实验入口，其自动加载尚未解决。[插件说明](addins/windows/README.md)
