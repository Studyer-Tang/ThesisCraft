...
存在 `dist/ThesisCraft.v4.0.0.exe` 时使用打包程序；否则使用项目 `.venv` 中的 Python。工具栏在 Office 的加载项区域或兼容工具栏中显示，具体位置取决于宿主版本和界面模式。
...
4. 点击 **排版设置 / 桌面版** 调整参数，在桌面版点击 **保存为默认**；插件下次排版就会使用新配置。
5. 使用 **关闭排版插件** 卸载本次临时工具栏。插件不会关闭文档，也不会修改 Office 的宏安全策略。

这是通过 Windows COM 提供的**外部工具栏插件**：需先启动一次并保持进程运行，暂不随 Office 自动启动。它不是 Office 应用商店 Web Add-in，也不是嵌入文档的宏。启动后不需要网络服务。重复启动同一种宿主会提示已有插件。

Word/WPS 的多个独立进程同时打开时，插件会连接 Windows 返回的当前自动化实例。应在需要处理文档的实例中使用工具栏；不会广播修改所有窗口。
