# -*- coding: utf-8 -*-
"""Command line entry point for ThesisCraft."""
...
from .config import DEFAULT_CONFIG, FONT_SIZE_MAP, normalize_config
...
FONT_SIZE_NAMES = {
    value: label.split(" ", 1)[0] for label, value in FONT_SIZE_MAP.items()
}
...
CONFIG_DESCRIPTIONS = {
    ...
    "line_spacing": ("正文行距", "磅；line_spacing_unit=pt 时使用"),
    "line_spacing_unit": ("正文行距单位", "pt / multiple；默认 pt"),
    "line_spacing_multiple": (
        "正文行距倍数",
        "line_spacing_unit=multiple 时使用，默认 1.0",
    ),
    "margin_top": ("上边距", "厘米"),
    ...
    "title_line_spacing": ("题目行距", "磅；title_line_spacing_unit=pt 时使用"),
    "title_line_spacing_unit": ("题目行距单位", "pt / multiple；默认 pt"),
    "title_line_spacing_multiple": (
        "题目行距倍数",
        "title_line_spacing_unit=multiple 时使用，默认 1.0",
    ),
    "subtitle_line_spacing": ("副标题行距", "磅；subtitle_line_spacing_unit=pt 时使用"),
    "subtitle_line_spacing_unit": ("副标题行距单位", "pt / multiple；默认 pt"),
    "subtitle_line_spacing_multiple": (
        "副标题行距倍数",
        "subtitle_line_spacing_unit=multiple 时使用，默认 1.0",
    ),
    "left_indent_cm": ("段落左缩进", "厘米；paragraph_indent_unit=cm 时使用"),
    "right_indent_cm": ("段落右缩进", "厘米；paragraph_indent_unit=cm 时使用"),
    "paragraph_indent_unit": ("段落缩进单位", "cm / chars；默认 cm"),
    "left_indent_chars": ("段落左缩进", "字符；paragraph_indent_unit=chars 时使用"),
    "right_indent_chars": ("段落右缩进", "字符；paragraph_indent_unit=chars 时使用"),
    "first_line_indent_chars": ("首行缩进", "字符；默认 2"),
    "title_bold": ("题目加粗", "true 时强制题目加粗"),
    "h1_bold": ("一级标题加粗", "true 时强制一级标题加粗"),
    "h2_bold": ("二级标题加粗", "true 时强制二级标题加粗"),
    ...
    "table_size": ("表格字号", "pt"),
    "table_line_spacing": ("表格行距", "磅；table_line_spacing_unit=pt 时使用"),
    "table_line_spacing_unit": ("表格行距单位", "pt / multiple；默认 pt"),
    "table_line_spacing_multiple": (
        "表格行距倍数",
        "table_line_spacing_unit=multiple 时使用，默认 1.0",
    ),
...
def load_config_with_overrides(args):
    config, source = load_config(
        getattr(args, "config", None), getattr(args, "config_json", None)
    )
...
def format_paths(args):
    ...
    def completed(index, total, job, state, detail):
        ...
        if state == "success":
            ...
            print(f"{state}: {job.source}: {detail}", file=sys.stderr)

    initialized = _initialize_com_for_thread(log)
    try:
        with WPSAppManager(log) as manager:
            ...
            result = run_jobs(
                processor, jobs, overwrite=args.overwrite, on_result=completed
            )
    finally:
        _uninitialize_com_for_thread(initialized, log)
    if args.json:
        print(
            json.dumps(
                {**asdict(result), "exit_code": result.exit_code}, ensure_ascii=False
            )
        )
...
def run_tests(_args):
    from .legacy_tests import main as test_main
...
def add_config_override_args(parser):
    parser.add_argument(
        "--config", help="JSON 配置文件路径；未指定时自动读取当前目录 wfp_config.json"
    )
    parser.add_argument(
        "--config-json", help="内联 JSON 配置对象，会覆盖配置文件中的同名字段"
    )
    parser.add_argument(
        "--set", action="append", help="覆盖单个配置项，格式 key=value，可重复使用"
    )
    parser.add_argument(
        "--enable-table-formatting", action="store_true", help="启用表格内容自动调整"
    )
    parser.add_argument(
        "--disable-table-formatting", action="store_true", help="关闭表格内容自动调整"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--english-font",
        help="数字和字母字体；设置后会自动启用 use_custom_english_font",
    )
    parser.add_argument(
        "--normalize-punctuation", action="store_true", help="启用符号标准化"
    )
    parser.add_argument(
        "--disable-normalize-punctuation", action="store_true", help="关闭符号标准化"
    )


def build_parser():
    parser = argparse.ArgumentParser(
        description="ThesisCraft CLI：将 doc/docx/wps/txt/md 按公文排版规则格式化为 docx。"
    )
    parser.add_argument(
        "--version", action="version", version=f"%(prog)s {__version__}"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    from .academic.cli import add_parser
    add_parser(subparsers)

    fmt = subparsers.add_parser("format", help="格式化单文件、多文件或目录")
    fmt.add_argument("paths", nargs="*", help="输入文件或目录，可一次传入多个")
    fmt.add_argument(
        "-i", "--input", dest="inputs", action="append", help="输入文件或目录，可重复"
    )
    ...
    fmt.add_argument(
        "--soffice", help="LibreOffice soffice 可执行文件路径；未指定时自动查找"
    )
    fmt.add_argument(
        "--soffice-timeout",
        type=int,
        default=120,
        help="LibreOffice 单文件转换超时秒数",
    )
    fmt.add_argument(
        "-v", "--verbose", action="store_true", help="输出详细处理日志到 stderr"
    )
    fmt.add_argument(
        "--overwrite", action="store_true", help="允许覆盖已有输出；仍禁止覆盖输入原件"
    )
    fmt.add_argument("--json", action="store_true", help="输出机器可读的完整处理摘要")
    fmt.set_defaults(func=format_paths)

    show = subparsers.add_parser(
        "show-config", help="显示当前配置、默认配置说明和可选增强项"
    )
    add_config_override_args(show)
    show.set_defaults(func=show_config)

    save = subparsers.add_parser(
        "save-config", help="保存配置到当前目录 wfp_config.json 或指定路径"
    )
    add_config_override_args(save)
    save.add_argument(
        "-o", "--output", help="输出 JSON 配置文件路径，默认 ./wfp_config.json"
    )
    save.add_argument(
        "-v", "--verbose", action="store_true", help="输出配置来源到 stderr"
    )
    save.set_defaults(func=save_config)

    subparsers.add_parser(
        "install-help", help="显示 LibreOffice 安装提示"
    ).set_defaults(func=install_help)
