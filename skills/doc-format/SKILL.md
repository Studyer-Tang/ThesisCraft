...
3.0.0，作者 Study-Tang。独立安装请使用 `packaging/build_skill.py` 生成的
`dist/doc-format-3.0.0.zip`；源码仓库内的脚本复用根目录共享包。
默认保留原颜色和分页，不自动接受修订；遇到既有输出默认保留，可显式使用
`--overwrite` 覆盖旧输出，但永远不能覆盖输入原件。CLI 返回 2 表示有跳过或取消。
